...
def get_solution(expression : str) -> int:
    ...
    the new precedence rules for the + and * operations (+ before *).
    """
...
def evaluate_expression(expression : list) -> int:
    ...
    left to right.
    """
    # first take care of all the addition operations
    expression_without_sums = []
    idx = 0
    while idx < len(expression):
        item = expression[idx]
        if item == "+":
            if expression[idx-2] != "+":
                previous_number = expression[idx-1]
            else:
                previous_number = expression_without_sums[-1]
            del expression_without_sums[-1]
            sum_to_replace = previous_number + expression[idx+1]
            expression_without_sums.append(sum_to_replace)
            idx += 2
        else:
            expression_without_sums.append(item)
            idx += 1

    # then take care of all the multiplication operations
    solution = expression_without_sums[0]
    for idx, item in enumerate(expression_without_sums[1:]):
        if item == "*":
            solution *= expression_without_sums[idx+2]
        else:
            pass
